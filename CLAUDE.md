...
Cross-platform dotfiles repository (macOS and Linux/GNOME) for setting up a development environment. All configs use **Catppuccin Macchiato** (dark) / **Catppuccin Latte** (light) theme where supported, **JetBrains Mono** font with **Fira Code** and **Symbols Nerd Font Mono** fallbacks. Configured for **Go** (via Homebrew), **Python** (via `uv`), and **Node.js** (via `fnm`).
...
make local-overrides    # Render per-machine overrides from .local/source.toml into tracked git/zprofile/claude/codex configs and .zshrc aliases
...
- `CLAUDE.md` - Project-level guidance for AI agents in this repo (Claude Code native; Codex via `project_doc_fallback_filenames`). Not the user-level `.config/claude/CLAUDE.md`.
- `scripts/symlinks.sh` - Creates symlinks (uses `set -euo pipefail`; defines `symlink` helper; branches on `uname -s` for tlrc/vscodium)
- `scripts/local-overrides.py` - Renders per-machine overrides from `.local/source.toml` into tracked git/zprofile/claude/codex configs plus `.zshrc` aliases; reads base from `git show HEAD:<path>` each run so overrides never compound. See "Local overrides" below.
...
- `docs/resources.md` - Curated external reference sites (software/alternative discovery, version/EOL tracking, CLI + dev reference, online tools, theming, privacy); companion to applications.md
...
- `.zshrc` / `.zprofile` - Zsh config. `.zprofile` sets `BREW_PREFIX`, XDG base-dir vars, `GOPATH=$XDG_DATA_HOME/go` (Go doesn't honor XDG natively), `GOPRIVATE`, `RIPGREP_CONFIG_PATH`, and `VISUAL`/`EDITOR`. `.zshrc` re-detects `BREW_PREFIX` (Linux/macOS branch identical to `.zprofile`) only (defensively, for non-login interactive shells where `.zprofile` was not sourced); other env vars use `${XDG_STATE_HOME:-...}` style fallbacks where they matter. `.zshrc` keeps `HISTFILE` under `$XDG_STATE_HOME/zsh/history` and loads starship prompt, fnm, uv, fzf with bat preview, eza aliases, syntax-highlighting, autosuggestions. Ghostty and Terminal.app open login shells, so `.zprofile` runs in practice; non-login interactive shells (e.g. `zsh -i` inside scripts) lose `RIPGREP_CONFIG_PATH` / `VISUAL` / `EDITOR` / `GOPATH` / `GOPRIVATE` plus the `$GOPATH/bin` and `~/.local/bin` PATH prepends (so go-installed and `uv tool` binaries drop off PATH) unless `.zprofile` is sourced manually.
...
- **`.local.example.toml`** + **`scripts/local-overrides.py`** — if the tool grows a per-machine override (identity, private namespace, team marketplace, plugin, shell alias, etc.), extend the schema and the renderer. Tracked file stays neutral; real value lives in `.local/source.toml`.
...
When adding a standalone doc (a new `docs/*.md` that is not a symlinked or parsed config, e.g. `resources.md`), the tool checklist above does not apply; only two registrations do: a README `## Contents` link and a CLAUDE.md `## Repository structure` bullet.
...
- Tracked configs ship with neutral placeholders (`your.email@example.com`, `github.com/your-org/*`, no team plugins, no aliases). Overrides upgrade them.
- **Claude/Codex parity:** both take `[<tool>.marketplaces.<key>]` + `[<tool>].plugins` with identical fields (`source`/`repo`/`url`/`auto_update`). `render_claude` writes JSON (`extraKnownMarketplaces` + `enabledPlugins`); `render_codex` appends TOML (`[marketplaces.<key>]` with `source_type = "git"`, normalizing a github `repo` to `https://github.com/<repo>.git`, plus `[plugins."<id>"] enabled = true`). The Codex render only ADDS: it appends marketplaces/plugins not already in `.config/codex/config.toml` and never disables a tracked one. Built-in marketplaces (`claude-plugins-official`, `openai-curated`) must not be declared. `auto_update` is Claude-only (tracked Codex marketplace blocks carry only `source_type` + `source` per `docs/conventions.md`).
- **Aliases:** `[aliases]` (name = command) render as `alias` lines into the `# >>> local-overrides:aliases >>>` / `# <<<` marker block in `.zshrc`; values are single-quoted via `shlex.quote`, so `$(...)` expands at use time, not render time. Commit the marker block to HEAD before adding aliases (`render_aliases` reads the block from HEAD and errors if it is missing), same base-first contract as `.zprofile`'s `GOPRIVATE` line.
...
These are logical steps. `scripts/validate.sh` groups the shell-adjacent sanity checks under section comments numbered `7` and `7b` through `7g` (including `7b2`), so its inline numbering is intentionally not 1:1 with this list.
...
- **Sensitive-data trust boundary:** Neither agent configures path-based deny rules, by choice. A Claude `settings.json` `Read(...)` deny gives false confidence: the allowed `Bash(...)` readers (`bat`, `head`, `jq`, etc.) bypass it. Codex's `permissions.<name>.filesystem` deny-read could block at the filesystem layer (covering those readers too), but is left unused to keep one shared enforcement model across both agents. Actual protection is the model-level `## Sensitive Data` rule: `.config/claude/CLAUDE.md` for Claude, `.config/codex/AGENTS.md` for Codex. Same path set, same enforcement layer.
