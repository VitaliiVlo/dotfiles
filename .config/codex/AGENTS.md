...
- **Filesystem destruction.** `rm -rf` outside cwd, `git clean -f`/`-fd`/`-fdx` (eats `.env`/`.local/`/scratch), shell redirect (`>`, `>>`, `tee` without `-a`) overwriting tracked files when an `apply_patch` edit is cleaner, recursive `chmod`/`chown` outside cwd, disk-wiping (`dd`, `mkfs`, `wipefs`, `shred`, `parted`).
