...
MANDATORY user-level rules for Codex and similar coding agents. Apply every task. Every section REQUIRED. Violations require correction before next action.
...
- No AI attribution anywhere: no `Co-Authored-By`, no "Generated with Codex", no signature footers, no "as an AI" disclaimers. Covers commits, PRs, issues, comments, Slack, Confluence, docs, status reports.
...
- Decision-class trigger (ambiguity, load-bearing decision, suspicious choice, detected mismatch, out-of-scope finding) surfaced as a numbered prompt before acting, not guessed or silently normalized.
- Scratch artifacts written under `/tmp/codex`, never `/tmp` root or working directory.
...
- For ad-hoc shell only, prefer the modern CLI over its legacy equivalent: `rg` over `grep`, `fd` over `find`, `bat` over `cat`, `eza` over `ls`, `dust` over `du -sh`, `tofu` (OpenTofu) over `terraform`; `jq`/`yq` for JSON/YAML, `exiftool` for media metadata, `tldr` for quick reference. Probe with `which <tool>` / `<tool> --version` before assuming a tool is absent; when both modern and legacy are installed, use the modern one. DO NOT replace a repo-defined script or `make` target.
...
- MUST verify checkable facts before asserting; never present guessed or unchecked data as fact, and prefer current sources over training memory. Pick the channel that owns the fact: web (`web_search`, vendor docs), whatever MCP servers the session exposes (`context7` for library / framework / SDK / API / CLI docs; a repo's own domain servers for their data, e.g. issue tracker, observability, analytics, cloud), CLI (`gh api` for GitHub state, `tldr` for command usage, `<tool> --version` probes, plus the inspection tools above). When a load-bearing fact resists verification, flag it as unverified and ask per `## Recommendations` rather than guess.
- MUST use the repo's declared toolchain: honor version files (`.tool-versions`, `.node-version` / `.nvmrc`, `.python-version`, `go.mod`'s `toolchain` directive, `rust-toolchain.toml`), lockfiles, and any `mise` / `asdf` config; NEVER global or system installs. Stacks I run: Node via `fnm` (auto-switches on `cd`; `fnm install <ver>`; run `npm`/`npx` against the active selection, NEVER global Node). Python via `uv` (owns interpreters, venvs, deps: `uv python install/list`, `uv venv`, `uv sync`, `uv run <cmd>`; NEVER system `pip` or hand-rolled venvs; `uv add` requires explicit approval). Go via Homebrew, no per-project switcher. Other stacks (Rust, Java, Ruby, ...): use the repo's pinned manager (`cargo`/`rustup`, Maven/Gradle wrapper, `bundler`) and match the lockfile; dependency adds require approval per `## Destructive Actions`.
...
- Subagents (`Agent` / task-spawn tools):
  ...
  - Skip for single-call tasks, mid-run user checks (no UI), subagent chains (run inline), design / planning decisions, or to dodge an approval prompt. Parallel tool calls in one message are NOT subagents.
...
- DO NOT add AI attribution: no `Co-Authored-By`, no "Generated with Codex" footer, no signature lines, no "as an AI" disclaimers, no tool-crediting footers. Applies to commits, PRs, issues, comments, Slack, Confluence, docs, status reports, any other human-facing text.
...
Scope: applies to assistant-drafted human-facing prose. Does NOT apply to code, code comments, or these rule files (em-dashes here are bugs to fix on next pass, not license to mirror). When editing pre-existing repo docs whose author voice uses em-dashes (table cells, established README/AGENTS.md prose), MUST match existing punctuation. DO NOT strip as side effect of unrelated edits.

## Sensitive Data

FORBIDDEN. DO NOT search, read, edit, print, summarize, transmit, or commit secrets or private credentials.

**Tool-agnostic and path-based.** Applies to every tool that can read, print, summarize, parse, or transmit file contents: `Read`, every `Bash` reader (`bat`, `cat`, `head`, `tail`, `less`, `grep`, `rg`, `awk`, `sed`, `jq`, `yq`, `find -exec cat`, `fd --exec`), `Edit`, `Write`, web fetch, MCP servers, external uploads. `Bash(bat .env)` is the same violation as `Read(.env)`. Allowlists in agent configs do not grant permission; rule is on the path, not the tool.
...
Categories below require explicit in-conversation approval THIS TURN, even when rule files or prior approval would permit. Rule is on action category, not tool; when in doubt, ask. Prior turn's approval does NOT carry forward unless user granted standing permission for the category.

- **Filesystem destruction.** `rm -rf` outside cwd, `git clean -f`/`-fd`/`-fdx` (eats `.env`/`.local/`/scratch), shell redirect (`>`, `>>`, `tee` without `-a`) overwriting tracked files when an `apply_patch` edit is cleaner, recursive `chmod`/`chown` outside cwd, disk-wiping (`dd`, `mkfs`, `wipefs`, `shred`, `parted`). Allowlisted readers with an exec or write subform stay gated at those subforms even though the bare binary is allowed: `find -exec <cmd>` / `find -delete`, `awk 'BEGIN{system(...)}'` / `awk '...>file'`, GNU `sed` `e` / `w` (exec forms inherit the category of the command they run).
...
- **Subagent boundary.** Spawning a subagent (`Agent` tool) to perform any action above REQUIRES the same approval as inline. Sandbox / classifier checks subagent task descriptions at spawn and each tool call inside.
...
- **Unverifiable claim.** A load-bearing fact you can't confirm with web, an available MCP server, or CLI per `## Tools`. Present it as unverified and ask; DO NOT assert a guess as fact.
...
Format: numbered prose prompt, recommended option first (suffixed `(Recommended)`), 2-4 mutually-exclusive options one per line. Each option: effect + trade-off, one line. NEVER add "Other". Wait for pick before acting.
...
MUST use `/tmp/codex` for all temporary internal artifacts: scratch outputs, notes, drafts, intermediate files, throwaway scripts. DO NOT write ephemera to `/tmp` root or working directory. MUST create the directory on demand if missing (`mkdir -p /tmp/codex`). MUST clean up own files when task is done.
