...
- **Filesystem destruction.** `rm -rf` outside cwd, `git clean -f`/`-fd`/`-fdx` (eats `.env`/`.local/`/scratch), shell redirect (`>`, `>>`, `tee` without `-a`) overwriting tracked files when `Edit`/`Write` is cleaner, recursive `chmod`/`chown` outside cwd, disk-wiping (`dd`, `mkfs`, `wipefs`, `shred`, `parted`). Allowlisted readers with an exec or write subform stay gated at those subforms even though the bare binary is allowed: `find -exec <cmd>` / `find -delete`, `awk 'BEGIN{system(...)}'` / `awk '...>file'`, GNU `sed` `e` / `w` (exec forms inherit the category of the command they run).
